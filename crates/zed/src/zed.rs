...
pub fn initialize_workspace(app_state: Arc<AppState>, cx: &mut App) {
    ...
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        let rally_status_item = cx.new(|cx| project_brain_panel::RallyStatusItem::new(cx));
        ...
        workspace.status_bar().update(cx, |status_bar, cx| {
            status_bar.add_left_item(rally_status_item, window, cx);
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("ZED_ALLOW_EMULATED_GPU").is_err() {
        ...
        let message = format!(
            db::indoc! {r#"
            Rally IDE uses {} for rendering and requires a compatible GPU.
...
fn open_about_window(cx: &mut App) {
    ...
    cx.open_window(
        WindowOptions {
            titlebar: Some(TitlebarOptions {
                title: Some("About Rally IDE".into()),
                appears_transparent: true,
...
mod tests {
    ...
    fn test_bundled_settings_and_themes(cx: &mut App) {
        ...
        for theme_name in themes.list().into_iter().map(|meta| meta.name) {
            ...
            if theme.name.as_ref() == "Rally Dark" {
