...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Rally IDE"),
        "preview" => ("app-icon-preview.ico", "Rally IDE Preview"),
        "nightly" => ("app-icon-nightly.ico", "Rally IDE Nightly"),
        _ => ("app-icon-dev.ico", "Rally IDE Dev"),
    };
